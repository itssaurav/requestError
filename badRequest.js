...
module.exports = function badRequest(data, options) {
    ...
    if (req.wantsJSON || sails.config.hooks.views === false) {
        ...
        // Format the response and log the error
        var customResponse = {
            statusCode: 400,
        ...
        // If it is a validation error, simply say validation error
        if (_.isObject(data) && (data.hasOwnProperty('isJoi'))) {
            data = 'Validation error';
        }

        // Log this response
        sails.log('response-400', req.referenceId, {
            method: req.method,
            url: req.url,
            referenceId: req.referenceId,
            errorMessage: data
